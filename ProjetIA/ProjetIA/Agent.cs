    ...
    class Agent
    {
        ...
        public void agentRoutine()
        {
            if (this._beliefs.environment.IsTurnAgent)
            {
                ...
                //par défaut le meilleur mouvement est la colonne du milieux.
                int bestMove = 3;
                bool winningMove = false;

                //si il y a un mouvement gagnant, pas la peine de faire negaMax, on cherche la col et on met le pion dedans
                for (int i = 0; i < this._beliefs.environment.Width; i++)
                {

                        if (this.captor.canPlay(i) && this.captor.isAWinningMove(i, playerTurn.playerYellow))
                        {
                            bestMove = i;
                            winningMove = true;
                        }
